...
export default function PlanFeatures() {
  ...
  const [newPlan, setNewPlan]   = useState({ name:"", type:"NORMAL", desc:"", price:0, duration:365, features:[{key:"",val:""}] });

  const addFeatRow = (setter) => setter(p=>({...p,features:[...p.features,{key:"",val:""}]}));
  const updFeat = (setter, i, field, val) => setter(p=>({...p,features:p.features.map((f,fi)=>fi===i?{...f,[field]:val}:f)}));
  const savePlan = () => {
    if(editPlan){ setPlans(ps=>ps.map(p=>p.id===editPlan.id?{...p,...editPlan}:p)); setEditPlan(null); }
    else { setPlans(ps=>[...ps,{...newPlan,id:uid(),typeId:"pt1"}]); setShowNew(false); setNewPlan({name:"",type:"NORMAL",desc:"",price:0,duration:365,features:[{key:"",val:""}]}); }
  };
  const PlanForm = ({data, setter, title, onSave, onCancel}) => (
    <Card style={{ padding:24, animation:"fadeUp .2s ease", marginBottom:20 }}>
      ...
      <div style={{ marginBottom:16 }}>
        <div style={{ fontSize:12, color:P.txt2, fontWeight:500, marginBottom:8 }}>Features (key / value)</div>
        {data.features.map((f,i)=>(
          <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 32px", gap:8, marginBottom:7 }}>
            <input value={f.key} onChange={e=>updFeat(setter,i,"key",e.target.value)} placeholder="feature_key" style={{ background:P.bg, border:`1.5px solid ${P.border}`, borderRadius:6, padding:"7px 10px", color:P.txt, fontSize:12, fontFamily:"monospace", outline:"none" }} />
            <input value={f.val} onChange={e=>updFeat(setter,i,"val",e.target.value)} placeholder="value" style={{ background:P.bg, border:`1.5px solid ${P.main}66`, borderRadius:6, padding:"7px 10px", color:P.main, fontSize:12, fontFamily:"monospace", outline:"none" }} />
            <button onClick={()=>setter(p=>({...p,features:p.features.filter((_,fi)=>fi!==i)}))} style={{ background:"none", border:`1px solid ${P.border}`, borderRadius:6, cursor:"pointer", color:P.error, display:"flex", alignItems:"center", justifyContent:"center" }}><X size={13}/></button>
          </div>
        ))}
        <button onClick={()=>addFeatRow(setter)} style={{ fontSize:12, color:P.main, background:"none", border:`1.5px dashed ${P.mainM}`, borderRadius:6, padding:"5px 14px", cursor:"pointer", fontFamily:"Inter,sans-serif" }}>+ Add Feature</button>
      </div>
            ...
            return (
              <Card key={plan.id} style={{ overflow:"hidden", animation:"fadeUp .3s ease" }}>
                ...
                <div style={{ padding:"14px 20px" }}>
                  {plan.features.map((f,i)=>(
                    <div key={i} style={{ display:"flex", justifyContent:"space-between", marginBottom:7, fontSize:12 }}>
                      <span style={{ color:P.txt3 }}>{f.key}</span>
                ...
                <div style={{ padding:"0 20px 16px", display:"flex", gap:6 }}>
                  ...
                  <Btn small variant="danger" icon={<Trash2 size={11}/>}>Delete</Btn>
                </div>
              </Card>
            );
          })}
